...
def generate_pdf_bytes(html_content: str, brand_style: dict = None, include_footer: bool = True) -> bytes:
    """Generate PDF from content using ReportLab for professional output"""
    ...
    # Use Helvetica (built-in, supports most chars)
    font_name = "Helvetica"
    ...
    # Custom styles
    styles.add(ParagraphStyle('DocH1', fontName=font_bold, fontSize=18, textColor=PRIMARY, spaceBefore=8*mm, spaceAfter=3*mm, leading=22))
    styles.add(ParagraphStyle('DocH2', fontName=font_bold, fontSize=14, textColor=PRIMARY, spaceBefore=6*mm, spaceAfter=2*mm, leading=17))
    styles.add(ParagraphStyle('DocH3', fontName=font_bold, fontSize=11, textColor=HexColor('#334155'), spaceBefore=4*mm, spaceAfter=2*mm, leading=14))
    styles.add(ParagraphStyle('DocBody', fontName=font_name, fontSize=9.5, textColor=TEXT_DARK, leading=13, spaceAfter=1.5*mm))
    styles.add(ParagraphStyle('DocBold', fontName=font_bold, fontSize=9.5, textColor=TEXT_DARK, leading=13, spaceAfter=1.5*mm))
    styles.add(ParagraphStyle('DocBullet', fontName=font_name, fontSize=9.5, textColor=TEXT_DARK, leading=13, leftIndent=8*mm, bulletIndent=3*mm, spaceAfter=1*mm))
    styles.add(ParagraphStyle('DocFooter', fontName=font_name, fontSize=7, textColor=TEXT_MUTED, alignment=TA_CENTER, leading=9))

    story = []
    W = doc.width

    # Strip HTML tags if content is HTML, keep plain text/markdown
    content = html_content
    # If it looks like HTML, extract text
    if content.strip().startswith('<'):
        content = re_mod.sub(r'<br\s*/?>', '\n', content)
        content = re_mod.sub(r'<[^>]+>', '', content)
        content = content.strip()

    # Parse markdown-style content into ReportLab elements
    lines = content.split('\n')
    table_rows = []
    in_table = False
    chart_json_lines = None
    ...
    for line in lines:
        ...
        if not stripped:
            if in_table and table_rows:
                ...
                _render_table(story, table_rows, W, PRIMARY, BORDER, BG_LIGHT, TEXT_DARK, font_name, font_bold)
                table_rows = []
                in_table = False
        ...
        if stripped == '```chart':
            chart_json_lines = []
            continue
        if chart_json_lines is not None:
            if stripped == '```':
                try:
                    chart_data = json.loads('\n'.join(chart_json_lines))
                    chart_img = _render_chart_to_bytes(chart_data)
                    if chart_img:
                        from reportlab.platypus import Image as RLImage
                        img = RLImage(io_mod.BytesIO(chart_img), width=W*0.85, height=W*0.5)
                ...
                chart_json_lines.append(stripped)
                continue

        # Table separator line (---|---|---)
        if re_mod.match(r'^[\s|:-]+$', stripped) and '|' in stripped:
            continue
        ...
        # Bullet points
            ...
            story.append(Paragraph(f'&bull; {text}', styles['DocBullet']))
        # Numbered list
        elif re_mod.match(r'^\d+\.\s', stripped):
            text = re_mod.sub(r'^\d+\.\s*', '', stripped)
            text = _format_inline(text)
            num = re_mod.match(r'^(\d+)', stripped).group(1)
            story.append(Paragraph(f'{num}. {text}', styles['DocBullet']))
        # Bold line
        elif stripped.startswith('**') and stripped.endswith('**'):
            text = stripped[2:-2]
            story.append(Paragraph(text, styles['DocBold']))
        # Regular paragraph
        else:
    ...
    if include_footer:
        ...
        story.append(HRFlowable(width="100%", thickness=0.5, color=BORDER))
        story.append(Spacer(1, 2*mm))
        story.append(Paragraph('Generated by Universal Document Creator', styles['DocFooter']))
...
def _format_inline(text: str) -> str:
    ...
    # Italic
    text = re_mod.sub(r'\*(.*?)\*', r'<i>\1</i>', text)
    # Inline code
    text = re_mod.sub(r'`(.*?)`', r'<font face="Courier" size="8">\1</font>', text)
    # Escape XML special chars (but not our tags)
    text = text.replace('&', '&amp;').replace('<b>', '\x00b').replace('</b>', '\x00/b').replace('<i>', '\x00i').replace('</i>', '\x00/i').replace('<font', '\x00font').replace('</font>', '\x00/font')
    text = text.replace('<', '&lt;').replace('>', '&gt;')
    text = text.replace('\x00b', '<b>').replace('\x00/b', '</b>').replace('\x00i', '<i>').replace('\x00/i', '</i>').replace('\x00font', '<font').replace('\x00/font', '</font>')
    return text


def _render_table(story, rows, width, primary_color, border_color, bg_light, text_dark, font_name, font_bold):
    """Render a markdown table as a professional ReportLab table"""
    from reportlab.lib.units import mm
    ...
    # Right-align columns that look like numbers/currency
    for col in range(num_cols):
        for row_idx, row in enumerate(rows):
            if col < len(row):
                cell = str(row[col]).strip()
                if cell.startswith('$') or cell.startswith('-$') or cell.startswith('(') or cell.replace(',','').replace('.','').replace('-','').isdigit():
                    style_cmds.append(('ALIGN', (col, row_idx), (col, row_idx), 'RIGHT'))
...
def _render_chart_to_bytes(chart_data: dict) -> bytes:
    """Render chart data to PNG bytes using matplotlib"""
    try:
        import matplotlib
        matplotlib.use('Agg')
        ...
        colors = ['#2563eb', '#f97316', '#0891b2', '#8b5cf6', '#10b981', '#f43f5e', '#6366f1', '#14b8a6']
        ...
        if chart_type == "bar":
            ...
            ax.bar(labels, values, color=colors[:len(data)], edgecolor='white', linewidth=0.5)
            for i, v in enumerate(values):
                ax.text(i, v + max(values)*0.02, f'{v:,.0f}' if isinstance(v, (int, float)) else str(v), ha='center', va='bottom', fontsize=8, fontweight='bold')
        elif chart_type in ("line", "area"):
            labels = [d.get("label", d.get("name", "")) for d in data]
            values = [d.get("value", 0) for d in data]
            ax.plot(labels, values, color=colors[0], linewidth=2, marker='o', markersize=5)
            if chart_type == "area":
                ax.fill_between(range(len(values)), values, alpha=0.15, color=colors[0])
                ax.set_xticks(range(len(labels)))
                ax.set_xticklabels(labels)
        elif chart_type == "pie":
            names = [d.get("name", d.get("label", "")) for d in data]
            values = [d.get("value", 0) for d in data]
            ax.pie(values, labels=names, colors=colors[:len(data)], autopct='%1.1f%%', startangle=90)
...
async def get_document_pdf(doc_id: int, user=Depends(get_current_user)):
    ...
    # Prefer raw markdown content for ReportLab engine; fall back to html_content
    pdf_content = doc["content"] or doc["html_content"] or ""
    loop = asyncio.get_event_loop()
    pdf_bytes = await loop.run_in_executor(_executor, generate_pdf_bytes, pdf_content, brand_style)
...
Include charts when showing: revenue projections, market data, comparisons, growth trends, budget breakdowns, survey results, statistics, or any numerical data that benefits from visualization. Do NOT include charts for simple text documents like letters or creative writing."""
    ...
    try:
        ...
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(content)
        # Save PDF — pass raw content for ReportLab engine
        pdf_bytes = generate_pdf_bytes(content or html_content, brand_style)
...
async def export_as_pdf(
    ...
    # ReportLab engine handles charts, tables, and markdown natively — pass raw content
    has_own_footer = "Generated by Universal Document Creator" in content
    loop = asyncio.get_event_loop()
    pdf_bytes = await loop.run_in_executor(_executor, generate_pdf_bytes, content, brand_style, not has_own_footer)
